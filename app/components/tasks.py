...
def SCHEDULER_SENSOR_TASKS(entries):
   ...
   for entry in entries:
      ...
      if entry.operator_main_1 == "None" or entry.operator_main_1 == None:
         ...
         sensor_value = MQTT_GET_SENSORDATA(device.gateway, device.ieeeAddr, entry.sensor_key)

         if entry.operator_1 == "=" and not entry.value_1.isdigit():
            if sensor_value == entry.value_1:
               passing == True
         if entry.operator_1 == "=" and entry.value_1.isdigit():
            if int(sensor_value) == int(entry.value_1):
               passing == True                  
         if entry.operator_1 == "<" and entry.value_1.isdigit():
            if int(sensor_value) < int(entry.value_1):
               passing == True
